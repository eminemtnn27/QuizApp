                ...
                <ul class="list-group"> 
                    @if($quiz->finished_at)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            Son Katılım Tarihi
                            <span title="{{$quiz->finished_at}}" class="badge badge-secondary badge-pill">{{$quiz->finished_at->diffForFumans()}}</span>
                        </li>
                        @endif 
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            Soru Sayısı
                        ...
                        <div class="card-body">
                            ...
                            <ul class="list-group">
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center"> 
                                   ...
                                   <span @if (auth()->user()->id==$result->user_id) 
                                     class="text-success"@endif>{{$result->user->name}} </span> 
            ...
            {{$quiz->description}}
                <table class="table table-bordered mt-3">
                    ...
                    @foreach ($quiz->results as $result )
                        ...
                        <td scope="row">{{$result->point}}</td>
                        <td scope="row">{{$result->correct}}</td>
                        <td scope="row">{{$result->wrong}}</td>  
                        </tr>
                    @endforeach 
                    </tbody>
                </table>
